...
interface AddPropertyDialogProps {
  ...
  onSuccess: () => void;
  toast: (props: { variant?: "default" | "destructive"; title?: string; description?: string }) => void;
}
...
export const AddPropertyDialog = ({
  ...
  const handleAddProperty = async () => {
    ...
    const interiorUrls = getInteriorRoomFiles().map(f => f.url).filter(Boolean) as string[];
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl font-raleway">Add New Property</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="sm:col-span-2">
                <Label htmlFor="property_address" className="text-sm">Property Address *</Label>
                <Input
                  id="property_address"
              ...
              <div>
                <Label htmlFor="area" className="text-sm">Local Government (Area) *</Label>
                <Select 
                  value={formData.area} 
              ...
              <div>
                <Label htmlFor="typology" className="text-sm">Property Type *</Label>
                <Select value={formData.typology} onValueChange={(v) => setFormData({...formData, typology: v})}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select type" />
                  </SelectTrigger>
                  <SelectContent>
                    {["1 Bedroom", "2 Bedroom", "3 Bedroom", "4 Bedroom", "5 Bedroom", "Duplex", "Apartment", "Villa"].map(t => (
              ...
              <div>
                <Label htmlFor="number_of_units" className="text-sm">Units *</Label>
                <Input
                  id="number_of_units"
                  type="number"
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-base sm:text-lg">Property Images</h3>
            ...
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-gray-600 mb-2">Upload multiple interior photos</p>
                <label className="cursor-pointer inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 px-3">
                  ...
                  <input
                    ...
                    multiple
                    className="hidden"
                    onChange={(e) => handleBulkInteriorUpload(Array.from(e.target.files || []))}
                  />
                </label>
              </div>
              {getInteriorRoomFiles().length > 0 && (
                <div className="mt-4 space-y-2 max-h-40 overflow-y-auto">
                  {getInteriorRoomFiles().map(file => (
                    <div key={file.id} className="border rounded-lg p-2 flex items-center justify-between">
                ...
                <AccordionItem
                  ...
                  content={
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                      <StandardDetail title="Paint & Walls" text="Newly painted with premium washable matte finish. Walls must be smooth and free of defects." />
                      <StandardDetail title="Flooring" text="High-grade ceramic/porcelain tiles (min. 60x60cm) or premium wood laminate." />
                    </div>
                  }
                />
                 <AccordionItem
                  ...
                  content={
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <StandardDetail title="Guaranteed Supply" text="10 Hours Night: 7PM-5AM; 5 Hours Day: 10AM-3PM." />
                      <StandardDetail title="Systems" text="Solar & Inverter preferred. Soundproofed generators required." />
                    </div>
                  }
                />
                 <AccordionItem
                  ...
                  content={
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <StandardDetail title="Ventilation" text="Min. 15% window-to-wall ratio. Cross-ventilation required." />
                      <StandardDetail title="A/C" text="Mandatory split units in all bedrooms and living areas." />
                    </div>
                  }
                />
                <AccordionItem
                  ...
                  content={
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <StandardDetail title="Access" text="Paved access road. Internal compound interlocked." />
                      <StandardDetail title="Security" text="Secured walls (min 2.4m) with barb wire/electric fence." />
                    </div>
                  }
                />
              </CardContent>
          ...
          <div className="flex items-start space-x-2 p-4 border rounded-lg bg-blue-50">
            ...
            <div className="grid gap-1.5 leading-none">
              <Label htmlFor="consent" className="text-sm font-medium">Compliance</Label>
              <p className="text-xs text-gray-600">I confirm my property meets ACCESSS standards.</p>
            </div>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={handleClose} disabled={isAddingProperty}>Cancel</Button>
          <Button onClick={handleAddProperty} disabled={isAddingProperty || !consentGiven} className="bg-primary">
            {isAddingProperty ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : "Add Property"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500">
               {file.uploading ? "Uploading..." : file.error ? "Error" : "Uploaded ✓"}
            </p>
          </div>
        </div>
...
const AccordionItem = ({ title, icon: Icon, isOpen, onToggle, content }: AccordionItemProps) => (
  <div className="border rounded-lg overflow-hidden">
    <div className="bg-gray-100 p-3 flex justify-between items-center cursor-pointer hover:bg-gray-200" onClick={onToggle}>
